...
class ActionBarAnnounceBorderType(
    tickPeriod: Long? = null,
    val notifyEnter: Boolean? = null,
    val notifyExit: Boolean? = null,
) : ResetBorder(BorderType.ACTION_BAR_ANNOUNCE, tickPeriod) {

    companion object {
        @Suppress("unused")
        @JvmStatic
        fun deserialize(args: Map<String, Any>): ActionBarAnnounceBorderType {
            ...
            return ActionBarAnnounceBorderType(
                tickPeriod,
                notifyEnter,
                notifyExit
            )
    ...
    fun notify(plugin: Homerun, player: Player, entering: Boolean) {
        if (entering && notifyEnter == true) {
            player.sendActionBar(plugin.messages.get("border-enter"))
        } else if (!entering && notifyExit == true) {
            player.sendActionBar(plugin.messages.get("border-exit"))
        }
    }
}