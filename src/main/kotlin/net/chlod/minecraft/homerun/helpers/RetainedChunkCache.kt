...
class RetainedChunkCache(val plugin: Homerun, val resetRules: List<ResetRule>) {
        ...
        for (resetRule in resetRules) {
            ...
            for (parameters in resetRule.parametersList) {
                val world = plugin.server.getWorld(
                    parameters.world ?: plugin.server.worlds[0].name
                )
                if (world == null) continue
                cacheRetainedChunksForWorld(resetRule, parameters, world)
                ...
                if (parameters.netherBehavior == ResetParameters.DimensionResetBehavior.NORMAL) {
                    ...
                    cacheRetainedChunksForWorld(resetRule, parameters, netherWorld)
                }
                if (parameters.endBehavior == ResetParameters.DimensionResetBehavior.NORMAL) {
                    ...
                    cacheRetainedChunksForWorld(resetRule, parameters, endWorld)
                }
            }
        }
    ...
    private fun cacheRetainedChunksForWorld(resetRule: ResetRule, parameters: ResetParameters, world: World) {
        val retainedChunkSets = parameters.retainedChunks.map {
